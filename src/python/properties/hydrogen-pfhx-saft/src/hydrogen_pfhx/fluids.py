...
class FluidStream(object):
    ...
    def __init__(self, fluid, mass_flow_rate, backend='HEOS'):
        # define input attributes
        self.fluid = fluid
        self.mass_flow_rate = mass_flow_rate

        # setup abstract state
        self.abstract_state = CP.AbstractState(backend, fluid)
...
class HeliumNeon(FluidStream):
    def __init__(self, mass_flow_rate, helium_fraction):
        FluidStream.__init__(self, 'Helium&Neon', mass_flow_rate, backend='REFPROP')
...
class HeliumNeonSAFT:
    """
    Helium-Neon coolant stream using SAFT-VRQ-Mie EoS and Residual Entropy
    Scaling (RES) transport properties (Li et al. 2024).

    Drop-in replacement for the original HeliumNeon class. Provides the same
    public interface so that bvp_model.py, hexs.py, and other modules require
    zero changes.
    ...
    helium_fraction : float
        Mole fraction of helium in the He-Ne mixture (0 to 1).
    """

    def __init__(self, mass_flow_rate, helium_fraction):
        from hydrogen_pfhx import saft_hene

        self.fluid = 'HeliumNeon-SAFT'
        self.mass_flow_rate = mass_flow_rate
        self.helium_fraction = helium_fraction
        self.mole_fractions = np.array([helium_fraction, 1.0 - helium_fraction])
        self._saft = saft_hene

        # Environmental variables
        self.temperature = None
        self.pressure = None

        # Derived properties (populated by set_properties)
        self.molecular_mass = None
        self.molar_density = None
        self.mass_density = None
        self.viscosity = None
        ...
        # Internal feos state (set on each update_conditions call)
        self._state = None

    def update_conditions(self, temperature, pressure):
        """Update temperature [K] and pressure [Pa], then create feos State."""
        self.temperature = temperature
        self.pressure = pressure
        self._state = self._saft.create_state(
            temperature, pressure, self.helium_fraction
        )

    def set_properties(self):
        """Compute all thermodynamic and transport properties from current state."""
        props = self._saft.extract_thermodynamic_properties(
            self._state, self.helium_fraction
        )

        self.molecular_mass = props['molecular_mass']
        self.molar_density = props['molar_density']
        self.mass_density = props['mass_density']
        self.specific_heat_capacity = props['specific_heat_capacity']
        self.enthalpy = props['enthalpy']
        self.entropy = props['entropy']

        self.viscosity = self._saft.mixture_viscosity(
            self._state, self.mole_fractions
        )
        self.thermal_conductivity = self._saft.mixture_thermal_conductivity(
            self._state, self.mole_fractions
        )

        self.prandtl_number = (
            self.viscosity * self.specific_heat_capacity / self.thermal_conductivity
        )
        self.molar_flow_rate = self.mass_flow_rate / self.molecular_mass

        # Critical properties (linear mixing approximation -- only used for
        # reference, not in the core heat transfer calculation)
        z = self.mole_fractions
        # He: Tc = 5.1953 K, Pc = 227600 Pa  (CoolProp values)
        # Ne: Tc = 44.4918 K, Pc = 2678600 Pa
        self.critical_temperature = z[0] * 5.1953 + z[1] * 44.4918
        self.critical_pressure = z[0] * 227600.0 + z[1] * 2678600.0

    def calculate_velocity(self, cross_sectional_area):
        """Calculate fluid velocity [m/s] from mass flow and density."""
        self.velocity = self.mass_flow_rate / (
            self.mass_density * cross_sectional_area
        )
    ...
    def calculate_reynolds_number(self, characteristic_length):
        ...
        self.reynolds_number = (
            self.mass_density * self.velocity * characteristic_length / self.viscosity
        )

    def specify_phase(self, phase):
        """No-op for API compatibility. SAFT-VRQ-Mie handles phase internally."""
        pass

    def set_mole_fractions(self, z):
        """Update mole fractions (for API compatibility)."""
        self.mole_fractions = np.array(z)
        self.helium_fraction = z[0]
