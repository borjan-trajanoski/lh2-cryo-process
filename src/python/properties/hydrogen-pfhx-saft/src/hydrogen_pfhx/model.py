...
from hydrogen_pfhx import (fluids, catalysts, hexs, bvp_model, outputs, utils)
...
def model(configuration):
    ...
    if coolant_name == 'Hydrogen':
        ...
        transport_model = configuration['coolant'].get('transport_model', 'SAFT')
        if transport_model == 'ONeill':
            coolant = fluids.HeliumNeon(coolant_mass_flowrate_kps, helium_fraction)
        else:
            coolant = fluids.HeliumNeonSAFT(coolant_mass_flowrate_kps, helium_fraction)
    else:
    ...
    # Attach solver metadata for diagnostics
    results.attrs['converged'] = solution.success
    results.attrs['solver_message'] = solution.message
    results.attrs['residual_norm'] = float(np.max(np.abs(solution.rms_residuals))) if hasattr(solution, 'rms_residuals') else None
    results.attrs['coolant_model'] = getattr(coolant, 'fluid', coolant_name)
